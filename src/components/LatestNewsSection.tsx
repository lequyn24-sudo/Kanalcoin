...
const mockNews = [
  {
    ...
    tag: "Markets",
    time: "2h ago",
    slug: "ethereum-etf-inflows"
  },
  {
    id: 2,
    title: "Solana Foundation Announces New Hackathon",
    source: "Decrypt",
    views: "5.1k",
    tag: "Ecosystem",
    time: "4h ago",
  ...
  {
    id: 3,
    title: "US Lawmakers Propose Comprehensive Crypto Bill",
    source: "Bloomberg",
    views: "24.8k",
    tag: "Regulation",
    time: "6h ago",
  ...
  {
    id: 4,
    title: "Top 5 DeFi Yield Farming Strategies for 2024",
  ...
  {
    id: 5,
    title: "Hardware Wallet Sales Surge After Exchange Hack",
    source: "CryptoSlate",
    views: "15.2k",
    tag: "Security",
    time: "15h ago",
    slug: "hardware-wallet-sales"
  }
];
      ...
      <div className="soft-card overflow-hidden flex-1 flex flex-col">
        <div className="flex flex-col flex-1 h-full">
          {mockNews.map((news, idx) => (
            <Link key={news.id} href={`/news/${news.slug}`} className={`flex-1 group flex flex-col sm:flex-row sm:items-center justify-between p-4 hover:bg-white/[0.02] transition-colors ${idx !== mockNews.length - 1 ? 'border-b border-border-light' : ''}`}>
              <div className="flex items-start sm:items-center gap-4 mb-3 sm:mb-0">
                <div className={`w-10 h-10 rounded-full flex items-center justify-center text-white font-bold shadow-inner bg-gradient-to-br from-[#8AA0E5] to-[#344A83]`}>
                  N
                </div>
                <div>
                  ...
                  <div className="flex items-center gap-3 text-xs text-text-dim">
                    <span className="font-medium text-text-muted">{news.source}</span>
                    <span>•</span>
                    <span>{news.time}</span>
                  </div>
                </div>
